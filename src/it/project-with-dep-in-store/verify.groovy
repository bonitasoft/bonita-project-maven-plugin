...
File jarFile = new File( localRepo, 'org/bonitasoft/in-store/0.0.1-SNAPSHOT/in-store-0.0.1-SNAPSHOT.jar');
if ( !jarFile.isFile() ) {
    throw new FileNotFoundException( "Could not find installed JAR: " + jarFile );
}

File pomFile = new File( localRepo, 'org/bonitasoft/in-store/0.0.1-SNAPSHOT/in-store-0.0.1-SNAPSHOT.pom');
if ( !pomFile.isFile() ) {
